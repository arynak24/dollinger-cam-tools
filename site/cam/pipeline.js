// -------------------------------------------------------
// pipeline.js — Excel-only CAM clause processing pipeline
// -------------------------------------------------------
...
export async function buildClauseObject() {
    ...
    return {
        ClauseID: null,      // Assigned after SharePoint save
        LeaseID: null,       // Optional future link to LeaseLibrary
        Source: "TenantForm",
        ...
        Tags: document.getElementById("camTags").value
            .split(",")
            .map(t => t.trim())
            .filter(Boolean),
